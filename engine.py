    ...
    for _ in range(max_iterations):
        ...
        if nfa_too_soft:
            ...
            if (trial_result.uls_utilization > 1.0 or trial_result.fls_utilization > 1.0
                ...
                # D/t ratio (observed: D/t as low as ~4 for some turbine
                # sizes before this guard was added). If it's specifically
                # the stiff-side case, that's usually not safety-critical, so
                # accept the geometry instead of flagging non-convergence.
                if nfa_too_stiff:
                    _note_nfa_stiff_accepted(result)
                    converged = True
                break
